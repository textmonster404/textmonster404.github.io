...
Game.Launch=function()
{
	...
	'<div class="listing">&bull; golden clovers should spawn a bit more often</div>'+
...
Game.autoClick = function(times,power,interval) {
    var timesClicked = 0;
    window.setInterval(function() {
        if (timesClicked<times) {
            timesClicked++;
            Game.ClickCookie(0,power);
        }
    },interval*1000)
};

window.setInterval(function() {
    if (Game.onMenu !== Game.oldMenu) {
        if (Game.onMenu == "log") {
    ...
    Game.oldMenu = Game.onMenu;
    if (document.getElementById("secret-container").style.opacity == "1") {
        document.getElementById("secret-button").onMouseDown = "Game.secretButtonClicked++;Game.ClickCookie(0,Game.secretButtonClicked);document.getElementById('secret-container').style.display = 'none';";}
    else {document.getElementById("secret-button").onMouseDown = "";}
},1);
Game.secretButtonClicked = 0;
Game.resetToys = function() {
    if (Game.TOYS==1) {Game.TOYS=0} else {Game.TOYS=1};
}
